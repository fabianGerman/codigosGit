...
int w=30,h=30,bs=20,dir=2,ax=12,ay=10;int[]dx={0,0,1,-1},dy={1,-1,0,0};boolean gameover=false;
void setup() { size(600, 600); x.add(5); y.add(5); }
void draw() {  background(255);
  ...
  if (!gameover) {
    fill(255, 0, 0); rect(ax*bs, ay*bs, bs, bs); 
  if (frameCount%5==0) {
  ...
  for(int i=1;i<x.size();i++) if(x.get(0)==x.get(i)&&y.get(0)==y.get(i)) gameover=true;
      if (x.get(0)==ax && y.get(0)==ay) { ax = (int)random(0, w); ay = (int)random(0, h); }
      else { x.remove(x.size()-1); y.remove(y.size()-1); }
    }
  }else {
    fill(0); textSize(30); textAlign(CENTER); text("PERDISTE. Pulsa el espacio para comenzar otra partida.",width/2,height/2);
